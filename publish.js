...
var JSDOC = require('jsdoc')
    ...
    handle = require('jsdoc/util/error')
        .handle,
...
exports.publish = function(data, opts, tutorials) {
    ...
    fs.mkPath((outdir + "symbols/src").split("/"));
    fs.mkPath((outdir + "js").split("/"));
    fs.mkPath((outdir + "css").split("/"));
    fs.mkPath((outdir + "img").split("/"));
    ...
    try {
        var classTemplate = new JSDOC.JsPlate(env.opts.template + "class.tmpl");
        var classesTemplate = new JSDOC.JsPlate(env.opts.template + "allclasses.tmpl");
        var sourceTemplate = new JSDOC.JsPlate(env.opts.template + "source.tmpl");
    } catch (e) {
    ...
    for (var i = 0, l = classes.length; i < l; i++) {
        ...
        fs.saveFile(outdir + "symbols/", ((opts.unique) ? Link.filemap[symbol.alias] : symbol.alias) + env.opts.ext, output);
    }

    // create the source code files
    Link.base = "../../";
    publish.classesIndex = classesTemplate.process(classes); // kept in memory
    var files = opts.srcFiles;
    for (var i = 0, l = files.length; i < l; i++) {
        var file = files[i];
        var srcDir = outdir + "symbols/src/";
        var name = file.replace(/\.\.?[\\\/]/g, "")
            .replace(/[\\\/]/g, "_");
        name = name.replace(/\:/g, "_");
        var output = "";
        output = sourceTemplate.process({
            file: file,
            source: escapeHTML(fs.readFile(file))
        });

        fs.saveFile(srcDir, name + env.opts.ext, output);
    }
    ...
    try {
        var classesindexTemplate = new JSDOC.JsPlate(env.opts.template + "index.tmpl");
    } catch (e) {
        print(e.message);
        quit();
    }
    ...
    fs.saveFile(outdir, "index" + env.opts.ext, classesIndex);
    classesindexTemplate = classesIndex = classes = null;

    // create the file index page
    try {
        var fileindexTemplate = new JSDOC.JsPlate(env.opts.template + "allfiles.tmpl");
    } catch (e) {
        print(e.message);
        quit();
    }
    ...
    fs.saveFile(outdir, "files" + env.opts.ext, filesIndex);
    fileindexTemplate = filesIndex = files = null;

    // Twitter Bootstrap
    var dstDir;

    var jsPaths = ['jquery-1.7.2.min.js', 'accordion.js', 'google-code-prettify/prettify.js'];
    ...
    for (var i = 0; i < numOfJsPaths; i++) {
        jsPath = jsPaths[i];
        fs.copyFile(env.opts.template + 'static/' + jsPath, dstDir);
    }

    var cssPaths = ['common.css', 'bootstrap/css/bootstrap.min.css', 'google-code-prettify/prettify.css', 'print.css'];
    ...
    for (var i = 0; i < numOfCssPaths; i++) {
        cssPath = cssPaths[i];
        fs.copyFile(env.opts.template + 'static/' + cssPath, dstDir);
    }

    var imgPaths = ['bootstrap/img/glyphicons-halflings-white.png', 'bootstrap/img/glyphicons-halflings.png', 'img/classicons.png', 'img/class.png', 'img/interface.png', 'img/namespace.png'];
    ...
    for (var i = 0; i < numOfImgPaths; i++) {
        imgPath = imgPaths[i];
        fs.copyFile(env.opts.template + 'static/' + imgPath, dstDir);
    }
}
...
function include(path) {
    var path = env.opts.template + path;
...
function getParentNamespaces(symbol) {
    var namespaces = symbol.alias.split('.')
        .slice(0, - 1);
    var symbols = [];
